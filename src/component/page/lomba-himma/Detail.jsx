...
export default function MasterLombaDetail({ onChangePage, withID }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const dataMhs = await UseFetch(
          API_LINK + "MasterLomba/GetMahasiswaLomba",
          currentFilter
        );

        if (data === "ERROR" || dataMhs === "ERROR" || data.length === 0) {
          throw new Error("Terjadi kesalahan: Gagal mengambil data Lomba.");
        } else if (dataMhs.length === 0) {
          ...
          const formattedData = dataMhs.map((value) => {
            const newValue = { ...value };
            newValue.Tanggal = new Date(newValue.Tanggal)
              .toISOString()
              .split("T")[0];
            return {
              ...newValue,
